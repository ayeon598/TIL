...
class Article(models.Model):
    # AUTH_USER_MODEL : accounts 앱의 User 모델
    # accounts라는 다른 앱에 있기 때문에 settings를 통해 들고옴.
    # on_delete=models.CASCADE : User가 회원탈퇴하면 게시글도 함께 삭제
...
class Comment(models.Model):
    # article이 위에 있으므로 settings에서 가져올 필요없다.
    # accounts 앱에서 user를 가져오기 위해 settings를 사용.
    # 게시글과 댓글간의 관계(1:N)
