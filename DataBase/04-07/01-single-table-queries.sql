...
-- 10. 중복없이 조회
-- customers 테이블에서 중복없이 Country를 조회하고,
...
-- 16. BETWEEN과 정렬 문제
-- tracks 테이블에서 Bytes가 100000과 500000 사이인 트랙의
-- Name과 Bytes를 조회하고 Bytes 기준으로 정렬하는 쿼리를 작성하세요.
SELECT
  Name,
  Bytes
FROM
...
-- Bytes를 기준으로 내림차순 정렬하여 상위 7개만 조회하는 쿼리를 작성하세요.
SELECT
  TrackId,
  Name,
  Bytes
...
SELECT
  Composer,
  AVG(Milliseconds / 60000) AS avgOfMinutes
FROM
  tracks
GROUP BY
  Composer
HAVING
  avgOfMinutes < 10;
-- WHERE을 쓴다? --> 그룹 짓기도 전에 평균가격을 모르는 상태에서 필터링 => 에러가 난다
-- Having ==> 각 그룹의 평균가격을 계산한 이후에 평균 가격을 알고 있는 상태에서 필터링